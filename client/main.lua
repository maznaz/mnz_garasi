...
function StoreOwnedCarMenu()
	...
	if IsPedInAnyVehicle(playerPed,  false) then
		...
		ESX.TriggerServerCallback('esx_advancedgarage:storeVehicle', function(valid)
			if valid then
				TriggerServerEvent('mnz_garage:StatusRusak', carplate, true)
				if Config.SimpanDamageGarasi == true then
					TriggerServerEvent('mnz_garage:SaveStatusRusak', vehicleProps)
					StoreVehicle(vehicle, vehicleProps)
				end
			end
		end, vehicleProps)
...
function SpawnVehicle(vehicle, plate)
	ESX.Game.SpawnVehicle(vehicle.model, this_Garage.Spawner, this_Garage.Heading, function(callback_vehicle)
		...
		TriggerServerEvent('mnz_garage:StatusRusak', vehicleProps.plate, false)
		if Config.SimpanDamageGarasi == true then
			ESX.Game.SetVehicleProperties(callback_vehicle, {
				bodyHealh = vehicle.bodyHealh,
				engineHealth = vehicle.engineHealth
			})
			DamageSimpan(vehicle, callback_vehicle)
		end
...
--------- DAMAGE SAVE ---------
function DamageSimpan(vehicle, callback_vehicle)
