...
-- SIMPAN DAMAGE KENDARAAN

RegisterNetEvent('mnz_garage:SimpanStatusRusak')
AddEventHandler('mnz_garage:SimpanStatusRusak', function(vehicle)
...
RegisterNetEvent('mnz_garage:StatusRusak')
AddEventHandler('mnz_garage:StatusRusak', function(vehiclePlate, stored)
    local xPlayer = ESX.GetPlayerFromId(source)
    ...
    MySQL.Async.execute('UPDATE owned_vehicles SET `stored` = @stored WHERE `plate` = @plate', {
        ...
        if data == 0 then
            print('mnz_garage: No values changed') 
        end
    end)
end)
